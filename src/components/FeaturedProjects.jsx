                                    ...
                                    {item.acf.tools_used && (
                                        <div className='flex'>
                                            {item.acf.tools_used.map((tool, index) => (
                                                <p className = {`mr-2 my-4 p-1 px-2 rounded-lg font-bold border text-base ${isDarkMode ? `border-white ${lightMode}` : `border-black ${darkMode}`}`} key={index}>{tool}</p>
                                            ))}
                                        </div>
                                    )}
                                </div>
                                <section className='p-2 space-y-2 md:text-base'>
                                    <h3>{item.acf.heading_tab}</h3>
                                    <article>
                                        <p>{item.acf.information_description}</p>
                                    </article>
                                    <h3 className='pt-4'>{item.acf.notable_heading}</h3>
                                    {item.acf.features && item.acf.features.map((article, index) => (
                                        <article key={index}>
                                            <h4 className='font-sans italic pt-4'>{article.notable_feature_subheading}</h4>
                                            {article.project_videos && 
                                                <div className='rounded-lg'>
                                                    <video className='py-4' src={article.project_videos.url} controls muted loop></video>
                                                </div>
                                            }
                                            <p>{article.notable_information}</p>
                                        </article>
                                    ))}
                                </section>
                            </AccordionBody>
